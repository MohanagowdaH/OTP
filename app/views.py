...
def register(request):
    if request.method=="POST":
        ...
        request.POST['methodOtp']=="methodOtpsms"
    ...
    if request.method=="POST":
        ...
        if request.COOKIES.get('can_otp_enter')!=None:
            if(profile.otp==request.POST['otp']):
                ...
                return red
            return HttpResponse("You have entered wrong otp")
        return HttpResponse("10 minutes passed, your OTP has expired")        
...
def home(request):
    if request.COOKIES.get('verified') and request.COOKIES.get('verified')!=None:
        return HttpResponse(" verified. Loged in succesfully")
    else:
